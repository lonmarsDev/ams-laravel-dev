...
@section('page_header')
    ...
    <div class="col-md-3">
        {!! Form::open(array('url' => route('showExhibitorEvents'), 'method' => 'get')) !!}
        ...
        @if($events->count())
            @foreach($events as $event)
                <div class="col-md-6 col-sm-6 col-xs-12">
                    @include('ManageExhibitor.Partials.EventPanel')
                </div>
            @endforeach
        @else
            @if($search['q'])
            ...
            @else
                @include('ManageExhibitor.Partials.EventsBlankSlate')
            @endif
        @endif
    </div>
    <div class="row">
