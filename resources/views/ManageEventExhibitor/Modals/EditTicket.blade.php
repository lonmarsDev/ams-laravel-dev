...
$('#access_code').change(function () {
    $('.access_code_details').toggle(this.checked);
}).change();

$('.generate_code').on('click', function() {
    $.ajax({
        type:'GET',
        url:'/organiser/access_code_generator',
        success:function(data){
            $('.access_code').val(data.generated_code);
        }
    });
});
</script>
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="form-group">
                            <div class="custom-checkbox">
                                {!! Form::checkbox('access_code', 1,  empty($ticket->with_accesscodes) ? false : true , ['data-toggle' => 'toggle', 'id' => 'access_code']) !!}
                                <!-- <input  data-toggle="toggle" id="access_code" name="access_code" type="checkbox" value="1"> -->
                                <label for="access_code">Access Code</label>
                            </div>
                        </div>
                        <div class="form-group access_code_details" style="display: none;">
                            {!! Form::generateCode($ticket->access_code, null, null) !!}
                        </div>
                    </div>
                </div>
