...
class EventCheckoutController extends Controller
{
        ...
        try {
            ...
            foreach ($ticket_order['tickets'] as $attendee_details) {
                ...
                for ($i = 0; $i < $attendee_details['qty']; $i++) {
                    ...
                    //exhibitor start counter and dashboard
                    $ticket = Ticket::where('id',$attendee_details['ticket']['id'])->first();
                    
                    $event_stats_exhibitor = EventStatsExhibitor::firstOrNew([
                        'event_id' => $event_id,
                        'date'     => DB::raw('CURRENT_DATE'),
                        'access_code' => $ticket->access_code ,
                    ]);
                    $event_stats_exhibitor->increment('tickets_sold', $ticket_order['total_ticket_quantity']);

                    if ($ticket_order['order_requires_payment']) {
                        $event_stats_exhibitor->increment('sales_volume', $order->amount);
                        $event_stats_exhibitor->increment('organiser_fees_volume', $order->organiser_booking_fee);
                    }
                    //var_dump($ticket);die();
    ...
    public function postSubmitPromo(Request $request, $event_id, $slug = '', $preview = false) {
        ...
        $event_tickets = $event->tickets()->where('is_hidden', 0)->orderBy('sort_order', 'asc');
        $data = [
            'event'       => $event,
            'tickets'     => $event_tickets->get(),
            'is_embedded' => 0,
        ];
        /*
        ...
        $with_access_code = $event->tickets()->where('is_hidden', 0)->where('with_accesscodes', 1)->where('access_code', $request->get('access_code'))->orderBy('sort_order', 'asc')->get();
        
        if($with_access_code->isEmpty()) {
            $data['tickets'] = $event->tickets()->where('is_hidden', 0)->where('with_accesscodes', 0)->orderBy('sort_order', 'asc')->get();
            return view('Public.ViewEvent.EventPageTicketAccessCode', $data);
        }
        else {
            $first = $event->tickets()->where('is_hidden', 0)->where('with_accesscodes', 1)->where('access_code', $request->get('access_code'))->orderBy('sort_order', 'asc');
            $second = $event->tickets()->where('is_hidden', 0)->where('with_accesscodes', 0)->orderBy('sort_order', 'asc');
            $data['tickets'] = $second->union($first)->get();
