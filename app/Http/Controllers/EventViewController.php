    ...
    public function showEventHome(Request $request, $event_id, $slug = '', $preview = false)
    {
        ...
        if (!$preview && !Auth::check()) {
            ...
            if ( $access_code = $request->get('access_code') ) {
                $event_statsEhibitor = new EventStatsExhibitor();
                $event_statsEhibitor->updateViewCount($event_id , $access_code);
            }
        ...
        $access_code_data = $event_tickets->where('with_accesscodes', 1)->get();
        ...
        $data['access_code'] = $request->get('access_code');
        $data['slug'] = $slug;  
