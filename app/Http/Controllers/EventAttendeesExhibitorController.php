...
class EventAttendeesExhibitorController extends MyBaseController
{
    /**
    ...
    public function showAttendees(Request $request, $event_id)
    {
        ...
        if ($searchQuery) {
            $attendees = $event->attendees()
                ...
                ->where('ticket_id',$exhibitor_ticket->ticket_id)
            ...
            $attendees = $event->attendees()
                ->join('orders', 'orders.id', '=', 'attendees.order_id')
                ->where('attendees.ticket_id',$exhibitor_ticket->ticket_id)
                ->withoutCancelled()
                ->orderBy(($sort_by == 'order_reference' ? 'orders.' : 'attendees.') . $sort_by, $sort_order)
                ->select('attendees.*', 'orders.order_reference')
                ->paginate();
        ...
        return view('ManageEventExhibitor.Attendees', $data);
    }
    ...
    public function showInviteAttendee(Request $request, $event_id)
    {
        ...
        $exhibitor_ticket = Exhibitor_ticket::where('event_id',$event_id)
                            ->where('exhibitor_id', $exhibitor->id)
                            ->first();
        $ticket = Ticket::where('id',$exhibitor_ticket->ticket_id)->lists('title', 'id');
        return view('ManageEventExhibitor.Modals.InviteAttendee', [
            'event'   => $event,
            'tickets' => $ticket,
        ]);
    }
    ...
    public function postInviteAttendee(Request $request, $event_id)
    {
        ...
        try {
            ...
            return response()->json([
                'status'      => 'success',
                'redirectUrl' => route('showEventExhibitorAttendees', [
                    'event_id' => $event_id,
                ]),
            ]);
    ...
    public function showPrintAttendees($event_id)
    {
        ...
        $data['attendees'] = $data['event']->attendees()->where('ticket_id',$exhibitor_ticket->ticket_id)->withoutCancelled()->orderBy('first_name')->get();
