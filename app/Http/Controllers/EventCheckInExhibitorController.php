...
class EventCheckInExhibitorController extends MyBaseController
{
    /**
    ...
    public function showCheckIn($event_id)
    {
        ...
        return view('ManageEventExhibitor.CheckIn', $data);
    }
    ...
    public function postCheckInAttendee(Request $request)
    {
        ...
        /*
        * store latest checkin/checkout information
        */
        ...
        DB::table('checkin_log')->insert(
            ['event_id' => $attendee->event_id, 'attendee_id' => $attendee_id, 'type' => ($checking == 'in') ? 1 : 0,'created_at'=>Carbon::now(),'updated_at'=>Carbon::now()]
        );
    ...
    public function postCheckInAttendeeQr($event_id, Request $request)
    {
        ...
        if ($attendee->has_arrived && $attendee->arrival_time->format('d m Y') == date('d m Y')) {
            return response()->json([
                'status'  => 'error',
        ...
        $organiser = User::where('account_id',$event->account_id)->first();
        /*
        * get organizer phone number and send a message.
        */
        if(!is_null($organiser->phone)){
            $message = $attendee->first_name." ".$attendee->last_name." checked in at ".$attendee->arrival_time->format('H:i A, F j');
            try{
                $twilio = Twilio::message($organiser->phone, $message);
            }
            catch (\Exception $e) {
                return response()->json([
                            'status'  => 'success',
                            'message' => 'Success !<br>Name: ' . $attendee->first_name . ' ' . $attendee->last_name . '<br>Reference: ' . $attendee->reference . '<br>Ticket: ' . $attendee->ticket . '.' . $appendedText.". Please note that the organizer could not be notified"
                        ]);
            }
        }
